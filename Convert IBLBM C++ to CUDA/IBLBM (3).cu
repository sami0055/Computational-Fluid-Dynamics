...
// __constant__ int NUM_CYLINDERS_CONSTANT = NUM_CYLINDERS;
...
int main() {
  ...
  for(int t = 1; t <= t_num; ++t) { // run over all times between 1 and t_num
    ...
    spread_kernel<<<(Nx * (Ny - 2) + 255) / 256, 256>>>(particles,force_x, force_y);
    lbm_kernel<<<(Nx * (Ny - 2) + 255) / 256, 256>>>(pop, pop_old, density, velocity_x, velocity_y, force_x, force_y);
    interpolate_kernel<<<(NUM_CYLINDERS * 36 + 255) / 256, 256>>>(particles,density,velocity_x, velocity_y, force_x, force_y);
