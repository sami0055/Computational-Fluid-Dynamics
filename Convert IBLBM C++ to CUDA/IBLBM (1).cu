...
__global__ void momenta_kernel(double *dev_pop, double *dev_density, double *dev_velocity_x, double *dev_velocity_y, int Nx, int Ny);
...
__global__ void momenta_kernel(double *dev_pop, double *dev_density, double *dev_velocity_x, double *dev_velocity_y, int Nx, int Ny) {
    ...
    if (i < Nx && j < Ny - 2) {
        int idx = j * Nx + i;
        dev_density[idx] = dev_pop[idx] + dev_pop[idx + Nx] + dev_pop[idx + 2 * Nx] + dev_pop[idx + 3 * Nx] + dev_pop[idx + 4 * Nx] + dev_pop[idx + 5 * Nx] + dev_pop[idx + 6 * Nx] + dev_pop[idx + 7 * Nx] + dev_pop[idx + 8 * Nx];
        dev_velocity_x[idx] = (dev_pop[idx + Nx] + dev_pop[idx + 5 * Nx] + dev_pop[idx + 8 * Nx] - (dev_pop[idx + 3 * Nx] + dev_pop[idx + 6 * Nx] + dev_pop[idx + 7 * Nx])) / dev_density[idx];
        dev_velocity_y[idx] = (dev_pop[idx + 2 * Nx] + dev_pop[idx + 5 * Nx] + dev_pop[idx + 6 * Nx] - (dev_pop[idx + 4 * Nx] + dev_pop[idx + 7 * Nx] + dev_pop[idx + 8 * Nx])) / dev_density[idx];
    }
}

void momenta() {
    dim3 blockSize(16, 16);
    dim3 gridSize((Nx + blockSize.x - 1) / blockSize.x, (Ny - 2 + blockSize.y - 1) / blockSize.y);
  
    double *dev_pop, *dev_density, *dev_velocity_x, *dev_velocity_y;
    cudaMalloc(&dev_pop, sizeof(double) * Nx * Ny * 9);
    ...
    cudaMemcpy(dev_pop, pop[0][0], sizeof(double) * Nx * Ny * 9, cudaMemcpyHostToDevice);
  
    momenta_kernel<<<gridSize, blockSize>>>(dev_pop, dev_density, dev_velocity_x, dev_velocity_y, Nx, Ny);
  
    cudaMemcpy(density[0], dev_density, sizeof(double) * Nx * Ny, cudaMemcpyDeviceToHost);
    cudaMemcpy(velocity_x[0], dev_velocity_x, sizeof(double) * Nx * Ny, cudaMemcpyDeviceToHost);
    cudaMemcpy(velocity_y[0], dev_velocity_y, sizeof(double) * Nx * Ny, cudaMemcpyDeviceToHost);
...
__global__ void compute_particle_forces_kernel(particle_struct *particles) {
 ...
 __global__ void initialize_particles_kernel(particle_struct *particles) {
  ...
  void write_fluid_vtk(int time) {
    ...
    cudaMemcpy(dev_density, density[0], sizeof(double) * Nx * Ny, cudaMemcpyHostToDevice);
    cudaMemcpy(dev_velocity_x, velocity_x[0], sizeof(double) * Nx * Ny, cudaMemcpyHostToDevice);
    cudaMemcpy(dev_velocity_y, velocity_y[0], sizeof(double) * Nx * Ny, cudaMemcpyHostToDevice);
    cudaMemcpy(dev_force_x, force_x[0], sizeof(double) * Nx * Ny, cudaMemcpyHostToDevice);
    cudaMemcpy(dev_force_y, force_y[0], sizeof(double) * Nx * Ny, cudaMemcpyHostToDevice);
